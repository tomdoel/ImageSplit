...
class ImageWrapper(object):
    """Multi-dimensional image array with an origin for reference to global coordinates"""
    ...
    def set_sub_image(self, sub_image):
        ...
        if np.any(np.less(start_indices, np.zeros(shape=start_indices.shape))) or \
                np.any(np.greater(end_indices, self.size)):
            raise ValueError("The sub image is not contained within the main image")
        selector = tuple([slice(start, end) for start, end in zip(start_indices, end_indices)])
